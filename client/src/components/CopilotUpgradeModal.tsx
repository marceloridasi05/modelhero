...
interface CopilotUpgradeModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export default function CopilotUpgradeModal({ open, onOpenChange }: CopilotUpgradeModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md" data-testid="dialog-copilot-upgrade">
        <DialogHeader className="text-center space-y-4">
          <div className="mx-auto flex items-center justify-center">
            <img 
              ...
              data-testid="img-copilot-upgrade-logo"
            />
          </div>
          <DialogTitle className="text-xl" data-testid="text-copilot-upgrade-title">
            {t('common.copilotUpgrade.limitReached')}
          </DialogTitle>
          <DialogDescription className="text-base" data-testid="text-copilot-upgrade-description">
            {t('common.copilotUpgrade.limitDescription')}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="bg-muted/50 rounded-lg p-4 space-y-3">
            ...
            <ul className="space-y-2">
              ...
              <li className="flex items-center gap-3">
                ...
                <span className="text-sm">{t("upgrade.features.copilot")}</span>
              </li>
            </ul>
          </div>

          <div className="text-center space-y-1">
            <p className="text-xs font-medium text-secondary" data-testid="text-copilot-upgrade-promo-label">
              {t(labelKey)}
            </p>
            <p className="text-2xl font-bold text-secondary" data-testid="text-copilot-upgrade-price">
              {originalPrice && (
        ...
        <DialogFooter className="flex-col gap-2 sm:flex-col">
          {useStripeCheckout ? (
            <Button 
              ...
              disabled={isLoading}
              data-testid="button-upgrade-copilot"
            >
              {isLoading ? (
          ...
          <Button 
            ...
            data-testid="button-close-copilot-upgrade"
          >
